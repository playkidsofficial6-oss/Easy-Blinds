...
export function JobCard({ job, isSelected, onSelect, onAction, variant = "assignment", showEditDelete = false }: JobCardProps) {
    ...
    return (
        <div
            ...
            className={cn(
                "group relative cursor-pointer rounded-[28px] border bg-white p-5 shadow-[0_12px_28px_rgba(15,23,42,0.08)] transition-all duration-200 active:scale-[0.99]",
        ...
        >
            <div className="flex items-start justify-between gap-3">
                <span className="rounded-lg border border-sky-100 bg-sky-50 px-3 py-1 text-[11px] font-black uppercase tracking-tight text-blue-700 shadow-sm">
                    {compactJobType(job)}
                </span>
                <div className="flex items-center gap-2">
                    <span className={cn(
                        "rounded-full px-4 py-1.5 text-[11px] font-black uppercase tracking-[0.16em] shadow-sm",
            ...
            <div className="mt-5">
                <div className="flex items-center gap-2">
                    <h4 className="min-w-0 flex-1 truncate text-[22px] font-black leading-tight tracking-[-0.03em] text-slate-900">
                        {job.client || "Unknown Unknown"}
                    </h4>
                    {job.priority === "High" && <span className="h-2.5 w-2.5 rounded-full bg-red-500 shadow-[0_0_0_5px_rgba(239,68,68,0.14)]" />}
                </div>
                <div className="mt-4 flex items-start gap-3 text-[16px] font-semibold leading-snug text-slate-400">
            ...
            <div className="flex items-center justify-between gap-3">
                <span className="text-[12px] font-black uppercase tracking-[0.16em] text-slate-400">Representative</span>
                <div className="flex max-w-[58%] items-center gap-2 rounded-full border border-slate-100 bg-white px-2.5 py-2 shadow-[0_4px_16px_rgba(15,23,42,0.05)]">
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white text-[14px] font-bold text-slate-950">{representativeInitials}</span>
                    <span className="truncate text-[15px] font-black text-slate-800">{representativeName}</span>
                </div>
            </div>

            <div className="my-5 h-px bg-slate-100" />

            <div className="rounded-[22px] border border-slate-100 bg-white px-4 py-4 shadow-[inset_0_0_0_1px_rgba(248,250,252,0.8)]">
                <div className="flex items-center justify-between gap-3 text-[15px] leading-none">
                    <div className="flex items-center gap-2 font-semibold text-slate-400">
                        <CarFront className="h-4 w-4 text-red-500" />
                        <span>Distance:</span>
                    </div>
                    <span className="font-semibold italic text-slate-400">{distanceLabel}</span>
                </div>
                <div className="mt-4 flex items-center justify-between gap-3 text-[15px] leading-none">
                    <div className="flex items-center gap-2 font-semibold text-slate-400">
                ...
                <div className="flex items-center justify-between gap-3 text-[15px] leading-none">
                    <div className="flex items-center gap-2 font-black uppercase tracking-[0.12em] text-slate-400">
            ...
            <div className="mt-4 flex items-center justify-between gap-3">
                ...
                {variant === "assignment" && (
                    <button
                        ...
                        className="rounded-full bg-slate-900 px-4 py-2 text-[11px] font-black uppercase tracking-wide text-white shadow-lg transition-colors hover:bg-amber-600"
                    >
            ...
            {isSelected && variant === "assignment" && job.recommendedFitters && (
                <div className="mt-5 animate-in slide-in-from-top-2 fade-in duration-300 border-t border-slate-100 pt-4">
