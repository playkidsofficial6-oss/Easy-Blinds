...
export default function SmartSalesmanAssignmentsPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)] overflow-hidden bg-slate-50 w-full font-sans">
      ...
      <div className="bg-white/95 border-b border-slate-200/80 px-6 py-3 flex items-center justify-between flex-shrink-0 z-30 shadow-sm backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="h-2.5 w-2.5 rounded-full bg-amber-500 animate-pulse"></div>
          <div>
            <h1 className="text-base font-bold text-slate-800 tracking-tight">Sales Operations Dispatch</h1>
            <p className="text-[10px] text-slate-400 font-semibold tracking-wider uppercase mt-0.5">Control Center</p>
          </div>
        </div>
        ...
        <div className="flex items-center bg-slate-100/80 p-1 rounded-xl border border-slate-200/50">
          <button
            onClick={() => { setActiveTab("fleet"); setSelectedJobId(null); setSelectedMapFitter(null); }}
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all",
              activeTab === "fleet" ? "bg-white text-amber-700 shadow-sm border border-slate-200/35" : "text-slate-500 hover:text-slate-800"
            )}
          >
          ...
          <button
            onClick={() => { setActiveTab("appointments"); setSelectedJobId(null); setSelectedMapFitter(null); }}
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all relative",
              activeTab === "appointments" ? "bg-white text-blue-700 shadow-sm border border-slate-200/35" : "text-slate-500 hover:text-slate-800"
            )}
          >
            <CalendarCheck className="w-3.5 h-3.5" />
            <span>Appointments</span>
            {filteredAppointments.length > 0 && (
              <span className="ml-1.5 bg-blue-100 text-blue-800 rounded-md px-1.5 py-0.5 text-[9px] font-bold">
                {filteredAppointments.length}
              </span>
            )}
          </button>
          <button
            onClick={() => { setActiveTab("leads"); setSelectedJobId(null); setSelectedMapFitter(null); }}
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all relative",
              activeTab === "leads" ? "bg-white text-amber-700 shadow-sm border border-slate-200/35" : "text-slate-500 hover:text-slate-800"
            )}
          >
            <Target className="w-3.5 h-3.5" />
            <span>Unassigned Leads</span>
            {filteredUnassignedLeads.length > 0 && (
              <span className="ml-1.5 bg-amber-100 text-amber-800 rounded-md px-1.5 py-0.5 text-[9px] font-bold">
                {filteredUnassignedLeads.length}
              </span>
            )}
          </button>
          <button
            onClick={() => { setActiveTab("measuring"); setSelectedJobId(null); setSelectedMapFitter(null); }}
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all relative",
              activeTab === "measuring" ? "bg-white text-indigo-700 shadow-sm border border-slate-200/35" : "text-slate-500 hover:text-slate-800"
            )}
          >
            <Ruler className="w-3.5 h-3.5" />
            <span>Measuring</span>
            {measuringSalesmen.length > 0 && (
              <span className="ml-1.5 bg-indigo-100 text-indigo-800 rounded-md px-1.5 py-0.5 text-[9px] font-bold">
                {measuringSalesmen.length}
              </span>
            )}
          </button>
          <button
            onClick={() => { setActiveTab("performance"); setSelectedJobId(null); setSelectedMapFitter(null); }}
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all",
              activeTab === "performance" ? "bg-white text-emerald-700 shadow-sm border border-slate-200/35" : "text-slate-500 hover:text-slate-800"
            )}
          >
        ...
        {/* Current Date Picker Info */}
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-between bg-slate-50 p-1 rounded-lg border border-slate-200/50">
            {/* <div className="flex items-center gap-1">
              <Button variant="ghost" size="sm" className={cn("text-[10px] font-bold h-6 px-2.5 rounded-md transition-all uppercase tracking-wider", isToday ? "bg-white shadow-sm text-slate-800 border border-slate-200/30" : "text-slate-500 hover:text-slate-700")} onClick={() => setViewDate(new Date())}>Today</Button>
              <Button variant="ghost" size="sm" className={cn("text-[10px] font-bold h-6 px-2.5 rounded-md transition-all uppercase tracking-wider", isTomorrow ? "bg-white shadow-sm text-slate-800 border border-slate-200/30" : "text-slate-500 hover:text-slate-700")} onClick={() => setViewDate(addDays(new Date(), 1))}>Tomorrow</Button>
            </div>
            <div className="flex items-center gap-1.5 pl-2 border-l border-slate-200 mx-2">
              <span className="text-xs font-bold text-slate-700">{format(viewDate, "MMM do")}</span>
              <Popover>
                <PopoverTrigger asChild><Button variant="ghost" size="icon" className="h-5 w-5 text-slate-400 hover:text-slate-600 rounded-full"><CalendarDays className="w-3 h-3" /></Button></PopoverTrigger>
